...
class ActionBlueprint:
    ...
    def validate(self) -> None:
        ...
        for i, sentence in enumerate(self.phrases):
            ...
            if len(inv_keys) > 0:
                raise ValueError(
                    f'In sentence {i} invalid keys were found: {', '.join(inv_keys)}. Sentence: {sentence}'
                )

        valid_player_placeholders = [
            "{" + role + "}" for role in get_player_valid_placeholders()
        ]

        if (
            self.scorer_player is not None
            and self.scorer_player not in valid_player_placeholders
        ):
            raise ValueError(f"Invalid goal_player placeholder: {self.scorer_player}")

        if (
            self.assist_player is not None
            and self.assist_player not in valid_player_placeholders
        ):
...
def extract_keys_from_format_string(format_str) -> list[str]:
    ...
    keys = [
        field_name for _, field_name, _, _ in formatter.parse(format_str) if field_name
    ]
