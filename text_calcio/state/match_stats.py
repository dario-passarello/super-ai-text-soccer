...
class TeamStats:
    ...
    def create_from_match(match : Match, team_id : Literal[0, 1]):
        ...
        n_attempts = sum(1 for action in all_actions if action.team_atk_id == team_id and action.phase != Match.Phase.PENALTIES)
        score = match.get_current_score()[team_id]
        goals = [
            GoalStats.create_from_action(action)
            for action in all_actions if action.team_atk_id == team_id
        ]
