...
class CLIDisplay:
    ...
    def display_minute(self):
        ...
        additional_time = self.match.curr_minute - self.match.curr_phase.duration_minutes

        base_minute = min(self.match.curr_phase.duration_minutes, self.match.curr_minute)
        ...
        if additional_time > 0:
            final_str += f' + {additional_time}:00  (+{self.match.get_added_time_minutes()})'
    ...
    def display_header(self):
        ...
        display = tabulate.tabulate([[t1_fmt, f'{score_t1} - {score_t2}', t2_fmt, self.display_minute()]], headers=[], tablefmt='rounded_grid')
        return display
    ...
    def display_after_goal(self):
        ...
        display = tabulate.tabulate(data, headers=headers, tablefmt='rounded_grid', colalign=['left', 'center', 'right'])
        return display

    def display_evalations(self):
        stats = MatchStats.create_from_match(self.match)
        tm_1_eval = [(pl, ev, 0) for pl, ev in stats.team_1_stats.player_evaluation.items()]
        tm_2_eval = [(pl, ev, 1) for pl, ev in stats.team_2_stats.player_evaluation.items()]
        ...
        headers = ['Player', 'Evaluation']
        display = tabulate.tabulate(data, headers=headers, tablefmt='rounded_grid', colalign=['left','right'])
        return display
    ...
    def display(self) -> tuple[str, list[list[str]]]:
        ...
        return header, [formatted_phrases[:i + 1] for i in range(len(formatted_phrases))]
...
"""     
        ...
        direction_names = [_('top left corner'), _('left on the ground'), _('center below crossbar'), _('center on the ground'), _('top right corner'), _('right on the ground')]

        position_text = '\n'.join([random_option] + [f'{i + 1} - {position}' for i, position in enumerate(direction_names)])
        ...
        while q0_remain:
            ...
            print()
            print(format_phrase(_('A penality was awarded to {atk_team_name}. Who is going to kick it?'), team_atk, team_def, assigments))
            player_list = '\n'.join([random_option] + [f'{i + 1} - {name}' for i, (role, name) in enumerate(atk_assigments)])
            print(player_list)
            q0_choice = await ainput('> ')
            try:
                q0_int = int(q0_choice)
                if not 0 <= q0_int <= len(team_atk.players):
                    raise ValueError(_('Number must be between 0 and ') + str(len(team_atk.players)))
                if q0_int == 0:
        ...
        print(self.display_header())
        print()
        await ainput(format_phrase(_('Now all the players and supporter of {def_team_name} must close their eyes or look away from the screen.'), team_atk, team_def, assigments))
        print(CLEAR_CHAR)
        print(self.display_header())
        print()
        ...
        while q1_remain:
            ...
            print(position_text)
            print()
            print(format_phrase(_('{}, where are you kicking the ball?').format(kicker_placeholder), team_atk, team_def, assigments))
            q1_choice = await ainput('> ')
            try:
                q1_int = int(q1_choice)
                if not 0 <= q1_int <= len(direction_names):
                    raise ValueError(_('Number must be between 0 and ') + str(len(team_def.players)))
                if q1_int == 0:
        ...
        print(self.display_header())
        print()
        await ainput(format_phrase(_('Now all the players and supporter of {atk_team_name} must close their eyes or look away from the screen.'), team_atk, team_def, assigments))
        print(CLEAR_CHAR)
        print(self.display_header())
        print()
        ...
        while q2_remain:
            ...
            print(position_text)
            print()
            print(format_phrase(_('{def_goalie} where are you diving?'), team_atk, team_def, assigments))
            q2_choice = await ainput('> ')
            try:
                q2_int = int(q2_choice)
                if not 0 <= q2_int <= len(direction_names):
                    raise ValueError(_('Number must be between 0 and ') + str(len(direction_names)))
                if q2_int == 0:
...
def format_phrase(
    phrase: str, atk_team: Team, def_team: Team, assignments: Mapping[str, str], attrs = []
):
    formatted_dict = {}
...
def format_minute(phase: Match.Phase, minute : int):
    ...
    elif phase > Match.Phase.SECOND_EXTRA_TIME:
